...
describe('Physics 2D Plugin', () => {
	...
	describe('Collision — Layer Filtering', () => {
		...
		test('physics collision respects collisionLayer', () => {
			const ecs = ECSpresso
				.create()
				.withPlugin(createTransformPlugin())
				.withPlugin(createPhysics2DPlugin({ gravity: { x: 0, y: 0 }, layers: filterLayers }))
				.withFixedTimestep(FIXED_DT)
				.build();
	...
	// ==================== Integration with Transform Plugin ====================

	describe('Integration with Transform Plugin', () => {
...
describe('Physics2D type narrowing', () => {
	test('Physics2D + Collision compose with narrow types', () => {
		...
		const ecs = ECSpresso
			.create()
			.withPlugin(createTransformPlugin())
			.withPlugin(createPhysics2DPlugin({ gravity: { x: 0, y: 100 }, layers }))
