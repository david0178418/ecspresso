...
describe('Spatial Index Bundle — Integration', () => {
	const layers = defineCollisionLayers({ player: ['enemy'], enemy: ['player'] });

	test('createSpatialIndexBundle installs spatialIndex resource', () => {
		const ecs = ECSpresso
			.create()
	...
	test('custom phases option limits to specified phases', () => {
		const ecs = ECSpresso
			...
			.withBundle(createTransformBundle())
			.withBundle(createCollisionBundle({ layers }))
			.withBundle(createSpatialIndexBundle({ phases: ['postUpdate'] }))
			.withFixedTimestep(1 / 60)
			.build();
...
describe('Spatial Index — Collision Bundle Integration', () => {
	const layers = defineCollisionLayers({ player: ['enemy'], enemy: ['player'] });

	test('collision bundle with spatial index produces same events as without', () => {
	...
	test('collision bundle without spatial index still works (brute-force fallback)', () => {
		const ecs = ECSpresso
			.create()
			.withBundle(createTransformBundle())
			.withBundle(createCollisionBundle({ layers }))
			// No spatial index bundle
			.build();
