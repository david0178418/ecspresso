/**
 * Particle System Bundle for ECSpresso
 *
 ...
 *
 * Follows the established bundle pattern: immutable shared config
...
import type { WorldTransform } from 'ecspresso/bundles/transform';
...
/**
 * Component types provided by the particle bundle.
 */
...
export interface ParticleBundleOptions<G extends string = 'particles'> {
...
/**
 * Create a particle system bundle for ECSpresso.
 *
 * Provides:
...
>(
	...
	bundle.registerRequired('particleEmitter', 'localTransform' as keyof ParticleComponentTypes, (() => ({
	...
	bundle.registerDispose('particleEmitter', (_emitter: ParticleEmitter, entityId: number) => {
	...
	bundle
		...
		.setProcess((queries, deltaTime, ecs) => {
			for (const entity of queries.emitters) {
				...
				// Read world transform for emission origin (cross-bundle structural access)
		...
		.setOnInitialize(async (ecs) => {
			...
			ecs.addReactiveQuery('particle-emitters', {
				...
				onEnter: (entity) => {
					...
					// Add to scene (cross-bundle structural access for renderLayer)
